<h2>Vetores (array)</h2>
<p>
    São estruturas de armazenamento compostas hetero gêneas;
    que podem ser indexadas númericamente ou textualmente
</p>

<?php
    //exemplo variavel simples
    $nota = 6;
    $nota = 3;
    echo $nota . "<br>";

    //exemplo vetor indexado numericamente
    $notas = []; //outra forma $nota = array();
    $notas[] = 6;
    $notas[] = 3;
    //var_dump($nota);
    //imprimir so a primeira nota
    echo ($notas[0] + $notas[1]) / 2 ;

    //Vetores com indice textual
    $notas = []; //cria um vetor vazio 
    $notas["debora"] = 7;
    $notas['murilo'] =7.8;
    $notas['poliana'] = 9;
    echo "<p></p>";
    echo "A Poliana tirou nota " . $notas['poliana'];

    //criar vector com numeros
    $valores = [];
    $valores[] = 2;
    $valores[] = 4;
    $valores[] = 8;
    $valores[] = 10;
    $valores[] = 12;
    $valores[] = 14;
    $valores[] = 16;

    //ou - $valores = [2, 4, 8, 10, 12, 14, 16];

    //imprimindo o conteudo de um vetor numerico.
    echo "<br>";
    for($i = 0; $i <= 6; $i++){
    echo $valores[$i] . "<br>";
    }

    //somar os valores de um vector
    $acumulador = 0;
    for($i=0; $i<=6; $i++){
        $acumulador =  $acumulador + $valores[$i]; 
    }
    echo "<br> A some dos valores é $acumulador";
    
    ?>

